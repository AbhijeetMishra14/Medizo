...
import { findUserByEmail, createUser, findUserById } from "../services/userService";
...
authRouter.post("/google", async (req, res) => {
  try {
    ...
    if (!user) {
      user = await createUser({ email, name, role: "user" });
    }
...
authRouter.post("/facebook", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: `Facebook login failed: ${err.message}` });
  }
});
    ...
    if (isMongo()) {
      ...
      if (isAdminApp && user.role === 'admin') {
        return res.status(403).json({ 
          message: "Administrators must use the admin dashboard to log in. Please use the Medizo Admin application."
        });
      }
